...
let allaudio = document.querySelector(".allaudio");
let contentImg = document.querySelector("#contentImg");
let body = document.querySelector("body");
let box2999 = document.querySelector(".box2999");
let allmusicbox22 = document.querySelector(".allmusicbox22");
let fullpagebox = document.querySelector(".fullpagebox");
let searchValue = document.querySelector("#searchValue");
let allmusicbox = document.querySelector(".allmusicbox");
let sewrchBtn = document.querySelector("#sewrchBtn");
let veiwallmusic = document.querySelector(".veiwallmusic");
...
allmusicbox22.innerHTML = musicbox
  .map((a, b) => {
    return `<div id="searchmusic" class="musicbox">
    <img height="100%" width="15%" src="${a.photo}">
    <b id="name">${a.songName}</b></div>`;
  })
  .join("");
  if( allmusicbox22.innerHTML !== ""){
    let searchmusic = document.querySelectorAll("#searchmusic");
    searchmusic.forEach((a,i) => {
  a.addEventListener("click", () => {
     fullpagebox.style.display = "flex"
     let song = a.innerHTML;
      musicbox.map((music, i) => {
        if (song.includes(music.songName)) {
          h4.innerHTML = `${musicbox[i].songName}`;
          audio.src = `${musicbox[i].audio}`;
  ...
  setInterval(function() {
    ...
    let searchmusic = document.querySelectorAll("#searchmusic");
 searchmusic.forEach((a,b) => {
 let aaa = a.innerHTML.toUpperCase();
a.style.display = "none"
 if(aaa.includes(searchval)){
a.style.display = "flex"
  ...
  sewrchBtn.addEventListener("click", () => {
    let searchval = searchValue.value.toUpperCase();
 let searchmusic = document.querySelectorAll("#searchmusic");
 searchmusic.forEach((a,b) => {
 let aaa = a.innerHTML.toUpperCase();
...
a.style.display = "flex"
  ...
  function closeAllMusic(){
    allmusicbox.style.display = "none"
    fullpagebox.style.display = "flex"
  }
  veiwallmusic.addEventListener("click", ( ) => {
    menubox.style.left = "-120%";
    fullpagebox.style.display = "none"
    allmusicbox.style.display = "flex"
    index.src = "Polish_20230823_145657371.png";
  })